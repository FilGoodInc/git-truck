...
export const Chart = memo(function Chart({ setHoveredObject }: { setHoveredObject: (obj: GitObject | null) => void }) {
  ...
  const { chartType, sizeMetric, groupingType, depthType, hierarchyType, labelsVisible, renderCutoff, fileGroups, selectedFilePaths, fileAuthorMode } = useOptions() // Remove selectedAuthorNames
  const { path } = usePath()
  ...
  const nodes = useMemo(() => {
    ...
    return res
  }, [size, chartType, sizeMetric, groupingType, path, renderCutoff, databaseInfo, filetree, fileGroups, selectedFilePaths, fileAuthorMode])
...
function createPartitionedHiearchy(
  ...
  // Declare castedTree once at the top
  let castedTree: GitObject
